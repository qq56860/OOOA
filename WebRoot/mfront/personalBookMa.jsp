...
<body><div id="onner" style="position:fixed; width:100%; height:100%;top:0;left:0; z-index:500;background:rgba(0,0,0,0.8); display:none"></div><div class="container">
 ...
 <form class="form-search"  style=margin-top:15px action="personal/findByPageConditions3?currentPage=${page.currentPage}&id=${m.mId }"  method="post">
                    ...
                    <tr>
                     <%--  <td>${P.pId }</td> --%>
                      ...
                      <td>${P.pAddress }</td>
  					  ...
  					  <button class="btn btn-info btn-sm" type="button" onclick="show('${P.pId}','${P.pName}','${P.pTel}','${P.pAddress}')"><i class="glyphicon glyphicon-edit icon-white"></i>修改</button>
					  </td>
                  	</tr>
                  	</c:forEach>
                  </tbody>
    ...
    <c:choose>
		<c:when test="${page.hasPrePage}">
			<a href="<%=path %>/personal/list3?id=${m.mId }&currentPage=1&from=1">首页</a> | 
			<a href="<%=path %>/personal/list3?id=${m.mId }&currentPage=${page.currentPage -1 }&from=1">上一页</a>
		</c:when>
		<c:otherwise>
			首页 | 上一页
		</c:otherwise>
	...
	<c:choose>
		<c:when test="${page.hasNextPage}">
			<a href="<%=path %>/personal/list3?id=${m.mId }&currentPage=${page.currentPage + 1 }&from=1">下一页</a> | 
			<a href="<%=path %>/personal/list3?id=${e.eId }&currentPage=${page.totalPage }&from=1">尾页</a>
		</c:when>
		<c:otherwise>
			下一页 | 尾页
		</c:otherwise>
  ...
  <div class="personal" align="center">
  		<form method="post" action="personal/update1?currentPage=${page.currentPage}&id=${m.mId }&pId=${p.pId}">
  			...
  			<div style="margin-top:50px;">
  				<input class="btn" type="submit" style="margin-right:55px;">
  				<input class="btn btn-info" type="button" value="关闭" onclick="hide()">
  			</div>
  		</form>		
  </div>
  <div class="addPersonal" align="center">
  		<form method="post" action="personal/add1?currentPage=${page.currentPage}&id=${m.mId }">
  			<div style="margin-top:80px;">联系人姓名：<input id="pname" type="text" name="pname"></div>
  			<div style="margin-top:30px;">联系人电话：<input id="ptel" type="text" name="ptel"></div>
  			<div style="margin-top:30px;">联系人地址：<input id="paddress" type="text" name="paddress"></div>
  			<div style="margin-top:50px;">
  				<input class="btn btn-info" type="submit" value="增加" style="margin-right:55px;">
  				<input class="btn btn-info" type="button" value="关闭" onclick="hidePersonal()">
  			</div>
  		</form>		
  </div>
  </div>      
